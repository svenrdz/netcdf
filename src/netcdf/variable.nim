import std/[strformat, sugar], ./[bindings, common, attribute]

type ValueTypes = cint | cfloat | cdouble
...
func getVar*(ds: Dataset, varId: int): Variable =
  ...
  handleError:
    ncinqvar(
      ds.id, varId.NcId, name.cstring, xtype.addr, ndim.addr, dimIds[0].addr, natt.addr
    )
  name.setLen(name.cstring.len)
  dimIds.setLen(ndim)
  result = Variable(dsid: ds.id, id: varId, name: name, xtype: xtype)
  result.dims = collect:
    for i in 0 ..< ndim:
      ds.dims[dimIds[i]]
  result.atts = collect:
...
func add*(ds: var Dataset, v: Variable) =
  ...
  handleError:
    ncdefvar(
      ds.id, v.name.cstring, v.xtype, dimIds.len.cint, dimIds[0].addr, varId.addr
    )
  v.dsid = ds.id
  v.id = varId
  ds.vars.add v
...
func `[]`*[T: ValueTypes](v: Variable, typ: typedesc[T], indices: varargs[uint]): T =
  assert indices.len == v.dims.len
  when T is cdouble:
    assert v.xtype == NcDouble
    handleError:
      ncgetvar1double(v.dsid, v.id.NcId, indices[0].addr, result.addr)
  elif T is cfloat:
    assert v.xtype == NcFloat
    handleError:
      ncgetvar1float(v.dsid, v.id.NcId, indices[0].addr, result.addr)
  elif T is cint:
    assert v.xtype == NcInt
    handleError:
      ncgetvar1int(v.dsid, v.id.NcId, indices[0].addr, result[0].addr)
...
func `[]`*[T: ValueTypes](
    v: Variable, typ: typedesc[T], slices: varargs[Slice[int]]
): seq[T] =
  ...
  when T is cdouble:
    assert v.xtype == NcDouble
    handleError:
      ncgetvaradouble(v.dsid, v.id.NcId, starts[0].addr, ends[0].addr, result[0].addr)
  elif T is cfloat:
    assert v.xtype == NcFloat
    handleError:
      ncgetvarafloat(v.dsid, v.id.NcId, starts[0].addr, ends[0].addr, result[0].addr)
  elif T is cint:
    assert v.xtype == NcInt
    handleError:
      ncgetvaraint(v.dsid, v.id.NcId, starts[0].addr, ends[0].addr, result[0].addr)
...
template `[]`*[T: ValueTypes](
    v: Variable, typ: typedesc[T], slices: varargs[typed, toSlice]
): seq[T] =
  v[typ, slices]
